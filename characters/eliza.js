...
function red_velvet() {
   ...
   person.description = 'Самый простой способ наложить противнику порчу, которая блокирует абилку. Несмотря на низкий урон, качают ей безкрит билд, и позиционируется она больше не как дамагер, а как дебаффер: также дают приём аппер для наложения оглушения. Качать можно для рифтов, но если у вас нет Жажды Смерти (Амбрелла).';
   person.ability = 'При переходе в режим Сехмет, накладывает врагу порчу на 15 сек. Если противник меняет персонажа под порчей, призванный получит 3 случайных дебаффа на 15 сек.';
   person.marquee1 = `${eliza.marquee1}`;
   person.marquee2 = `${eliza.marquee2}`;
   person.prestige_description = `${eliza.prestige}`;
   person.prestige_activation = `${eliza.p_active}`;
   ...
   person.def_stats = ['Не защитник'];
   person.build_1_img = 'eliza/Sekhmets_Bite.png';
   person.build_2_img = 'eliza/Sekhmets_Turn.png';
   person.build_3_img = 'eliza/Upper_Khat.png';
   person.build_4_img = 'eliza/Chaos_Banish.png';
   person.build_1_desc = 'Сехмет с шансом дать проклятье.';
   person.build_2_desc = 'Сехмет с шансом дать кровотечение и блок исцеления.';
...
function lapis_luxury() {
   ...
   person.description = 'Самый красивый скин в игре и высокая атака. И бесполезная абилка. Максимум, можно снизить своё здоровье до 1% с неупалом, и при активации бб3 поменяться здоровьем с врагом и убить его этой бб3; либо играть чисто уроном без абилки. Но качать не стоит.';
