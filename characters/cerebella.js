...
function big_top() {
   ...
   person.description = 'Серьёзный серебрянный танк, угроза для новичков, не знающих как её выйграть и чем проходить. Качать рекомендуется, алмазить тоже можно, хоть и слабая, но не дорогая защита лучше чем без защиты.';
   person.ability = 'Один раз за бой воскрешается с 50% здоровья с неуязвимостью и непоколебимостью 8 сек. При падении любого персонажа, снимает с себя 1 отрицательный эффект и получает мощную регенерацию на 10 сек.';
   person.marquee1 = `${cerebella.marquee1}`;
   person.marquee2 = `${cerebella.marquee2}`;
   person.prestige_description = `${cerebella.prestige}`;
   person.prestige_activation = `${cerebella.p_active}`;
...
function star_spangled() {
   ...
   person.description = 'Весьма странная карточка: атакующая абилка и большое здоровье. Но абилку реализовать сложно в воздухе и барьер ей не помогает держать защиту. Не рекомендуется прокачивать, очень слабый персонаж.';
   person.ability = 'При броске или его отмене, дает барьер себе и раскол брони врагу на 15 сек. Под барьером, и если враг в воздухе, шанс 15% при ударе дать кровотечение и усталость на 7 сек.';
   person.marquee1 = `${cerebella.marquee1}`;
   person.marquee2 = `${cerebella.marquee2}`;
   person.prestige_description = `${cerebella.prestige}`;
   person.prestige_activation = `${cerebella.p_active}`;
