...
function raw_nerv() {
   ...
   person.description = 'Впечатляющая вариация, дающая постоянную метку смерти врагу, также получающая неупал, под которым наносит больше урона и хиллится. Отлично сочетается с саппортами, также дающими неупал.';
   person.ability = 'Когда здоровье становится ниже 50%, даёт неупал на 15 сек себе и постоянную метку смерти врагу. С шансом 10% при нанесении удара под неупалом, получает бешенство и восстанавливает 50% здоровья от нанесённого урона.';
   person.marquee1 = `${painwheel.marquee1}`;
   person.marquee2 = `${painwheel.marquee2}`;
   person.prestige_description = `${painwheel.prestige}`;
   person.prestige_activation = `${painwheel.p_active}`;
